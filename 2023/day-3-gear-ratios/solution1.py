...
with open('input.txt', 'r') as file:
    ...
    for line in file.readlines():
        line = list(line[:-1])
        matrix.append([None if char == '.' else char for char in line])
...
def locate_symbols(matrix: np.array) -> (np.array, list):
    ...
    for i, row in enumerate(matrix):
        for j, elem in enumerate(row):
            if elem is not None and not elem.isdigit():
                list_symbols.append((i, j))
                matrix[i, j] = None
    return matrix, list_symbols


def add_numbers_symbol(matrix: np.array, symbols_pos: tuple) -> (np.array, int):
    total = 0
    for i in range(-1, 2):
        for j in range(-1, 2):
...
for symbol in list_symbols:
    matrix, partial_sum = add_numbers_symbol(matrix, symbol)
    total += partial_sum
print(total)
